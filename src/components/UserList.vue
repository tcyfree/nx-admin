...
<script >
  export default {
    ...
    methods:{
      ...
      fetchCustomers(){
        this.$http.get("http://api.go-qxd.com/admin/user_list?page="+ this.pageCurrent + "&size=" + this.pageSize)
      ...
      updateCustomer(id,status,$event){
        ...
        this.$http.put("http://api.go-qxd.com/admin/user/status",updateCustomer)
          .then(function(response){
            this.$router.push({path:"/user_list?page="+ this.pageCurrent + "&size=" + this.pageSize});
            alert('ok!');
